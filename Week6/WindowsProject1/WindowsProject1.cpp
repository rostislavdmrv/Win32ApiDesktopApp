...
INT_PTR CALLBACK MyFunction(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_INITDIALOG:
        c_index = SendDlgItemMessage(hDlg, IDC_COMBO1, CB_ADDSTRING, 0, (LPARAM)"test");
    ...
    case WM_COMMAND:
        ...
        else if (mID == IDC_BUTTON2)
        {
            ...
            if (cbIndex > -1)
            {
                ...
                {
                    MessageBox(hDlg, "Did not checked any boxes", "Error", MB_OK);
                }
                else if ((IsDlgButtonChecked(hDlg, IDC_CHECK1)))
                {
