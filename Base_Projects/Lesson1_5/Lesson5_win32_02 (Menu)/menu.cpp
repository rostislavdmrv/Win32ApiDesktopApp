                     ...
                     int       nCmdShow)
{
	...
	hAccelTable = LoadAccelerators(hInstance, (LPCTSTR)IDC_MENU);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, (LPCTSTR)IDI_MENU);
		...
		case WM_COMMAND:
			...
			switch (wmId)
			{
				...
				case IDM_ADD:
					{
						...
						if(GetMenuState(hMenu, IDM_ADD + 1, MF_BYCOMMAND) == -1) // or disable
						{
							MENUITEMINFO mii;
				...
				case IDM_ADD + 1:
					...
					break;
				case IDM_DO:
					MessageBox(hWnd, _T("Do Something"), _T("Message"), MB_OK);
					break;
				case IDM_ABOUT:
